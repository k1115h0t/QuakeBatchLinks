...
// @version      1.1.0
// @description  Batch open/copy Quake result links via passive response monitoring with asset-level deduplication.
// @description:zh-CN 被动读取 Quake Web 已有搜索响应，按资产去重并批量打开/复制链接；不主动发送或重放 Quake API 请求。
...
(() => {
  ...
  function emptyStats() {
    return { raw: 0, assets: 0, dedup: 0, links: 0, noLink: 0, invalid: 0 };
  }
  ...
  function isTarget(input, method) {
    ...
    return !!u && String(method || 'GET').toUpperCase() === 'POST' && u.origin === ORIGIN && u.pathname === PATH;
  }
  ...
  function assetKey(row, index) {
    ...
    const parts = [row?.ip, row?.domain, row?.port, row?.transport, row?.service?.name]
      .map(v => String(v ?? '').trim());
    return parts.some(Boolean) ? `fallback:${parts.join('|')}` : `row:${index}`;
  }
  ...
  function extract(payload) {
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      let urls = map.get(key);
      if (!urls) map.set(key, urls = new Set());
  ...
  function hookFetch() {
    ...
    try { W.fetch = wrapped; return true; }
    catch (e) { console.warn('[QuakeBatchLinks] fetch hook failed:', e); return false; }
  }
  ...
  function hookXhr() {
    ...
    try { p.open = wrappedOpen; p.send = wrappedSend; return true; }
    catch (e) { console.warn('[QuakeBatchLinks] XHR hook failed:', e); return false; }
  }

  function snapshotUrls() {
    const out = [];
    for (const asset of currentAssets) for (const url of asset) out.push(url);
    return out;
  }

  function copyAll() {
    const urls = snapshotUrls();
    if (!urls.length) return updateUi(null, '当前没有可复制的资产链接', true);
  ...
  async function openAll() {
    const urls = snapshotUrls();
    if (!urls.length) return updateUi(null, '当前没有可打开的资产链接', true);
    if (urls.length >= OPEN_CONFIRM && !W.confirm(
      `即将按资产打开 ${urls.length} 个标签页，是否继续？\n\n` +
      `原始 ${stats.raw} 条 · 唯一资产 ${stats.assets} 个 · 去重 ${stats.dedup} 条`
    )) { urls.length = 0; return; }

    updateUi(null, `正在打开 ${urls.length} 个资产链接…`, false);
    for (const url of urls) {
      GM_openInTab(url, { active: false, insert: true, setParent: true });
      if (OPEN_DELAY) await new Promise(r => setTimeout(r, OPEN_DELAY));
    }
    urls.length = 0;
  ...
  function updateUi(nextStats, message, error) {
    ...
    if (nextStats) {
      ...
      ui.stats.textContent = `原始 ${nextStats.raw} · 去重 ${nextStats.dedup} · 链接 ${nextStats.links} · 无链接 ${nextStats.noLink}`;
      ui.stats.title = `原始数据：${nextStats.raw}\n唯一资产：${nextStats.assets}\n被去重数据：${nextStats.dedup}\n可用链接：${nextStats.links}\n无链接资产：${nextStats.noLink}\n无效 URL：${nextStats.invalid}`;
    }
  ...
  function createUi() {
    ...
    box.innerHTML = `
      ...
      <div class="qbl-stats">原始 0 · 去重 0 · 链接 0 · 无链接 0</div>
    ...
    ui = {
      count: box.querySelector('.qbl-count'), stats: box.querySelector('.qbl-stats'),
      status: box.querySelector('.qbl-status'), open: box.querySelector('.qbl-open'), copy: box.querySelector('.qbl-copy')
    };
  ...
  console.info('[QuakeBatchLinks] passive hooks:', `fetch=${fetchHooked}, xhr=${xhrHooked};`, 'asset-level dedup enabled; no extra Quake API request is issued.');
})();
